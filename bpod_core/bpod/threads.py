...
class ReadThread(threading.Thread):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            q_events.put(RawEvent(perf_count_ns, start_micros_us, _EventID.START_FSM))
            q_events.put(RawEvent(perf_count_ns, start_micros_us, _EventID.START_STATE))
            ...
            q_events.put(
                RawEvent(perf_count_ns, derived_micros, _EventID.END_FSM_CYCLES)
            )
            q_events.put(
                RawEvent(perf_count_ns, end_micros_us, _EventID.END_FSM_MICROS)
            )

        finally:
            self._queue_events.put(RawEvent(0, 0, _EventID.STOP_SENTINEL))
...
class EventThread(threading.Thread):
    ...
    def stop(self) -> None:
        ...
        self.queue.put(RawEvent(0, 0, _EventID.STOP_SENTINEL))
        ...
        while True:
            ...
            if event_index < 255:
                ...
                if target_state == target_exit:  # state exited without a successor
                    self._append(
                        t_system_ns, t_bpod_us, _EventID.END_STATE, state=current_state
                    )
                    event_queue.task_done()
                    continue
                ...
                self._append(
                    t_system_ns, t_bpod_us, _EventID.START_STATE, state=current_state
                )
            ...
            if event_index < 255 or event_index == _EventID.START_STATE:
                ...
                for idx in sorted(active_outputs.keys() | new_actions.keys()):
                    ...
                    output_id = _OUTPUT_ID_OFFSET + idx
                    self._append(t_system_ns, t_bpod_us, output_id, val, current_state)
                active_outputs = {
                    idx: val
    ...
    def get_data(self) -> pl.DataFrame:
        ...
        n_synthetic = len(_SYNTHETIC_EVENT_TYPES)
        event_lookup = pl.DataFrame(
            {
                'event_id': pl.Series(
                    list(range(len(names))) + list(_SYNTHETIC_EVENT_TYPES) + output_ids,
                    dtype=pl.Int16,
                ),
                'type': (
                    ['InputEvent'] * len(names)
                    + list(_SYNTHETIC_EVENT_TYPES.values())
                    + ['OutputAction'] * len(output_names)
                ),
                'event': names + [None] * n_synthetic + [None] * len(output_names),
                'channel': (
                    [None] * len(names)
                    + [None] * n_synthetic
                    + list(self._action_names)
                ),
            }
        )
        ...
        return (
            ...
            .join(event_lookup, on='event_id', how='left')
            .join(state_lookup, on='state', how='left')
            .drop('system_time', 'event_id', 'state')
